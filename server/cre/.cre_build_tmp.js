...
function base64Decode2(b64) {
  ...
  for (let i2 = 0;i2 < clean.length; i2 += 4) {
    const a = table.indexOf(clean[i2]);
    const b = table.indexOf(clean[i2 + 1]);
    const c = table.indexOf(clean[i2 + 2] ?? "A");
    const d = table.indexOf(clean[i2 + 3] ?? "A");
    result += String.fromCharCode(a << 2 | b >> 4);
    if (i2 + 2 < clean.length)
      result += String.fromCharCode((b & 15) << 4 | c >> 2);
    if (i2 + 3 < clean.length)
      result += String.fromCharCode((c & 3) << 6 | d);
  }
  return result;
}
var httpTrigger = new HTTPCapability;
var httpClient = new ClientCapability3;
function verifyDocument(runtime2, payload) {
  const { hash } = decodeJson(payload.input);
  if (!hash || !/^[0-9a-f]{64}$/i.test(hash)) {
    throw new Error("Invalid hash: must be a 64-character SHA-256 hex string");
  }
  runtime2.log(`[SealSign] Verifying hash: ${hash.slice(0, 16)}...`);
  const aggregated = runtime2.runInNodeMode((nodeRuntime) => {
    const { mirrorNodeUrl, topicId } = nodeRuntime.config;
    const url = `${mirrorNodeUrl}/api/v1/topics/${topicId}` + `/messages?limit=100&order=asc`;
    const response = httpClient.sendRequest(nodeRuntime, { url, method: "GET" }).result();
    if (!ok(response)) {
      throw new Error(`Hedera Mirror Node returned HTTP ${response.statusCode}`);
    }
    const data = json(response);
    for (const msg of data.messages) {
      const decoded = base64Decode2(msg.message);
      let record;
      try {
        record = JSON.parse(decoded);
      } catch {
        nodeRuntime.log(`[SealSign] Skipping malformed message seq=${msg.sequence_number}`);
        continue;
      }
      if (record.hash.toLowerCase() === hash.toLowerCase()) {
        nodeRuntime.log(`[SealSign] Match found at sequence ${msg.sequence_number} ` + `issuer=${record.issuer}`);
    ...
    nodeRuntime.log(`[SealSign] No match found for hash ${hash.slice(0, 16)}...`);
    return JSON.stringify({
      verified: false,
      issuer: "",
      documentType: "",
  ...
  const result = JSON.parse(aggregated.result());
  runtime2.log(`[SealSign] Consensus reached: verified=${result.verified}`);
  return result;
}
function initWorkflow() {
  return [
    cre.handler(httpTrigger.trigger({}), verifyDocument)
  ];
}
async function main() {
