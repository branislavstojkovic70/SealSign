...
var confidentialHttpClient = new ClientCapability2;
function verifyDocument(runtime2, payload) {
  ...
  const { mirrorNodeUrl, topicId } = runtime2.config;
  const url = `${mirrorNodeUrl}/api/v1/topics/${topicId}` + `/messages?limit=100&order=asc`;
  const response = confidentialHttpClient.sendRequest(runtime2, {
    vaultDonSecrets: [],
  ...
  for (const msg of data.messages) {
    ...
    try {
      ...
      runtime2.log(`[SealSign] Skipping malformed message seq=${msg.sequence_number}`);
      continue;
    }
    if (record.hash.toLowerCase() === hash.toLowerCase()) {
      runtime2.log(`[SealSign] Match found at sequence ${msg.sequence_number} ` + `issuer=${record.issuer}`);
      return {
        verified: true,
  ...
  runtime2.log(`[SealSign] No match found for hash ${hash.slice(0, 16)}...`);
  return {
    verified: false,
    issuer: "",
    documentType: "",
